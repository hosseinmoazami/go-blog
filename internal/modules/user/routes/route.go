...
func Routes(router *gin.Engine) {
	...
	guestGroup.Use(middlewares.IsGuest())
	{
		...
		guestGroup.POST("/register", userController.RegisterHandle)

		guestGroup.GET("/login", userController.Login)
		guestGroup.POST("/login", userController.LoginHandle)
	...
	{
		authGroup.POST("/logout", userController.LogoutHandle)
	}
